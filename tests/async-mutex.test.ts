...
async function testAsyncMutex() {
  ...
  console.log(
    `   ✓ 转账成功: ${account1AfterTransfer?.balance === 2300 && account2AfterTransfer?.balance === 200}\n`
  );
  ...
  console.log(
    `   ✓ 冻结成功: ${frozenAccount?.balance === 2200 && frozenAccount?.frozenBalance === 100}\n`
  );
  ...
  console.log(
    `   ✓ 解冻成功: ${unfrozenAccount?.balance === 2250 && unfrozenAccount?.frozenBalance === 50}\n`
  );
