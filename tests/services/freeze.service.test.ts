...
describe('FreezeService', () => {
  ...
  describe('createInitialFreeze', () => {
    test('应该成功创建初始冻结', async () => {
      ...
      const freezeRecord = await freeze;
      expect(freezeRecord.id).toBeDefined();
      expect(freezeRecord.userId).toBe(userId);
      expect(freezeRecord.amount).toBe(500);
      expect(freezeRecord.type).toBe(FreezeType.INITIAL);
      expect(freezeRecord.status).toBe(FreezeStatus.FROZEN);
      expect(freezeRecord.transactionId).toBe('tx-001');
    });
