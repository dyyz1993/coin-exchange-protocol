...
import { FreezeType, FreezeStatus, FREEZE_CONFIG } from '../types';
export class FreezeService {
     ...
     * 手动解冻（指定原因）
     */
    manualUnfreeze(freezeId, reason) {
     ...
     * 根据交易ID查询冻结记录
     */
    getFreezeByTransactionId(transactionId) {
     ...
     * 延长冻结时间（争议场景）
     */
    extendFreeze(freezeId, durationMinutes) {
        const freeze = freezeModel.getFreeze(freezeId);
        if (!freeze) {
            throw new Error('冻结记录不存在');
        }
     ...
     * 批量解冻用户的指定冻结记录
     */
    unfreezeMultiple(freezeIds, reason) {
        const results = [];
