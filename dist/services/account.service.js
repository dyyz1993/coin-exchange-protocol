...
export class AccountService {
     ...
     * 创建新用户账户
     */
    async createAccount(userId, initialData) {
     ...
     * 增加用户代币（奖励、空投等）
     */
    async addTokens(userId, amount, type, description, relatedId) {
     ...
     * 扣除用户代币（消费、惩罚等）
     */
    async deductTokens(userId, amount, type, description, relatedId) {
     ...
     * 冻结用户代币
     */
    async freezeTokens(userId, amount, reason, relatedId) {
     ...
     * 解冻用户代币
     */
    async unfreezeTokens(userId, amount, reason) {
     ...
     */
    updateAccountInfo(userId, updates) {
     ...
     * 为 AccountController 提供的便捷方法
     */
    async freezeAccount(userId, reason, duration) {
        const account = accountModel.getAccountByUserId(userId);
        if (!account) {
            throw new Error('账户不存在');
        }
     ...
     * 为 AccountController 提供的便捷方法
     */
    async unfreezeAccount(userId, reason) {
        const account = accountModel.getAccountByUserId(userId);
        if (!account) {
            throw new Error('账户不存在');
        }
