...
export const FreezeManager = () => {
    ...
    const getStatusBadge = (status) => {
        ...
        if (status === 'rejected' || status === 'unfrozen')
    ...
    return (<div className="p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="flex gap-3 mb-6">
        <button onClick={() => setShowApplyForm(true)} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">
          📝 新建冻结申请
        </button>
      </div>
      ...
      <div className="flex gap-2 mb-6 border-b border-gray-200">
        <button onClick={() => setActiveTab('requests')} className={`px-4 py-2 font-medium transition-colors ${activeTab === 'requests'
            ? 'text-blue-600 border-b-2 border-blue-600'
            : 'text-gray-500 hover:text-gray-700'}`}>
          冻结申请 ({freezeRequests.length})
        </button>
        <button onClick={() => setActiveTab('records')} className={`px-4 py-2 font-medium transition-colors ${activeTab === 'records'
            ? 'text-blue-600 border-b-2 border-blue-600'
            : 'text-gray-500 hover:text-gray-700'}`}>
                ...
                {filteredRequests.map((request) => (<tr key={request.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {request.status === 'pending' && (<div className="flex gap-2">
                          <button onClick={() => handleApprove(request.id)} className="px-3 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200 text-sm">
                            通过
                          </button>
                          <button onClick={() => handleReject(request.id)} className="px-3 py-1 bg-red-100 text-red-700 rounded hover:bg-red-200 text-sm">
                            拒绝
                          </button>
                        </div>)}
                      {request.status !== 'pending' && (<span className="text-sm text-gray-400">已处理</span>)}
                    </td>
                  </tr>))}
              </tbody>
            </table>
      ...
      {/* 冻结记录列表 */}
      {activeTab === 'records' && (<div className="overflow-x-auto">
          <table className="min-w-full bg-white border border-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
            ...
            <tbody className="divide-y divide-gray-200">
              {freezeRecords.map((record) => (<tr key={record.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {record.status === 'active' ? (<button onClick={() => handleUnfreeze(record.id)} className="px-3 py-1 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 text-sm">
                        解冻
                      </button>) : (<span className="text-sm text-gray-400">已解冻</span>)}
                  </td>
                </tr>))}
            </tbody>
          </table>
      ...
      {/* 冻结申请表单弹窗 */}
      {showApplyForm && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleApply}>
              <div className="mb-4">
                ...
                <input type="text" value={applyData.accountId} onChange={(e) => setApplyData({ ...applyData, accountId: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="acc-001" required/>
              </div>
              <div className="mb-4">
                <label className="block text-gray-700 text-sm font-bold mb-2">
                  冻结金额
                </label>
                <input type="number" value={applyData.amount} onChange={(e) => setApplyData({ ...applyData, amount: parseFloat(e.target.value) })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00" min="0" step="0.01" required/>
              </div>
              <div className="mb-4">
                <label className="block text-gray-700 text-sm font-bold mb-2">
                  冻结原因
                </label>
                <textarea value={applyData.reason} onChange={(e) => setApplyData({ ...applyData, reason: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="请详细说明冻结原因..." rows={4} required/>
              </div>
              <div className="flex gap-3">
                ...
                <button type="button" onClick={() => setShowApplyForm(false)} className="flex-1 bg-gray-300 text-gray-700 py-2 rounded-lg hover:bg-gray-400">
                  取消
                </button>
              </div>
            </form>
